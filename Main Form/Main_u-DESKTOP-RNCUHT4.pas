...
type
  ...
  private
    iMinW, iMinH, icountpassword, iUser, iAnswer, iQuestion, ipass: integer;
    bMoving, bTeacher, bTest: boolean;
    arrEnglish: array [0 .. 9] of string;
    arrZulu: array [0 .. 9] of string;
...
begin
  ...
  begin
    bEnglish := Messages(
      'Would you like to take the course in English? No to take the course in Zulu.');
...
procedure TfrmMain.btnPassClick(Sender: TObject);
var
  iID: integer;
  bFound: boolean;
begin
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            tblAnswers.Post;
            bFound := true;
            showmessage('Table has been updated');
          end;
          tblAnswers.Next;
        end;
      end;
...
begin
  ...
  begin
    ...
    showmessage('file not find, contact teacher.');
end;
...
begin
  ...
  if btest = true then
  begin
    Showmessage('Well done! You have finished the benchmark test.');
  end;
end;
...
begin
  ...
  begin
    { sFile := ExtractFilePath(Application.ExeName) + 'mathcourse.mdb';
      ...
      if FileExists(ExtractFilePath(Application.ExeName) + 'mathcourse.mdb')
      then
      begin
      dmMathSharks.commathsharks.ConnectionString :=
...
begin
  ...
  pnlC.Caption := 'C. ' + inttostr(iCorrect + randomrange(0, 500));
  iAnswer := 1;
  tmrq.Enabled := true;
  prbTime.Max := 30;
...
begin
  ...
  begin
    ...
    begin
      ...
      case rgpSort.ItemIndex of
        ...
        4:
          tblusers.sort := 'Gender DESC';
        5:
          begin
            dbgDB.DataSource := dsshow;
            qryshow.Close;
            qryshow.SQL.Text :=
              'Select Username, Password, FirstName, Surname, DOB, Gender' +
  ...
  begin
    ...
    begin
      if ((sWord[i]) IN (['1', '2', '3', '4', '5', '6', '7', '8', '9', '0']))
        or (sWord[i] IN ['&', '@', '#', '$', '%', '!', '^', '*', '(', ')'])
        and bB = true then
      begin
        result := false;
    ...
    begin
      ...
      begin
        ...
        begin
          result := false;
          bV := false;
          i := MessageDlg('Username already taken :/ ' + #10 +
              '=================' + #10 + 'Maybe try this: ' + snew +
              ' press yes to use this username', mtCustom, [mbYes, mbNo], 0);
          case i of
            mrYes:
